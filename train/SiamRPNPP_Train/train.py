...
pretrain_model_path = save_folder + "/SiamRPNPP_epoch_5_coco.pt"
...
def train(model, device, optimizer, train_loader):
    ...
    for i, data in enumerate(train_loader):
        ...
        if i % 2000 == 0:
            t_video_save_model_path = video_save_model_path[:-3]+"_%d.pt" % (i)
            torch.save(model.state_dict(), t_video_save_model_path)
...
def main():
    ...
    if os.path.exists(pretrain_model_path):
        ...
        # model.load_state_dict(torch.load(pretrain_model_path, map_location={"cuda:2":"cuda:0"}))
