...
class Video:
    ...
    def get_data(self):
        # get frames path
        self.frames = []
        frame_names = [int(f[:-4]) for f in os.listdir(self.video_path) if f.endswith("jpg")]
        frame_names.sort()
        for i in range(len(frame_names)):
            frame_path = self.video_path+"/%04d.jpg" % (frame_names[i])
            self.frames.append(frame_path)
        # get gt
        self.gt = []
        self.gt = np.genfromtxt(self.video_path+"/groundtruth.txt", delimiter=",", dtype=int)



class TC_DataSet:

    def __init__(self, TC_path="E:/dataset/tc"):
        self.dataset_name = "TC"
    ...
    def get_data(self):
        self.videos = []
        video_names = os.listdir(self.TC_path)
        for i in range(len(video_names)):
            video_path = self.TC_path+"/"+video_names[i]
            self.videos.append(Video(video_path))


def show():
    dataset = TC_DataSet(TC_path)
    i = 0
    videos = dataset.videos
    # print(len(videos))
    # frame_cnt = 0
    # for i in range(len(videos)):
    #     frame_cnt+=len(videos[i].gt)
    # print(frame_cnt)
